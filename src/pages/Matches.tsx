...
export default function Matches() {
  const { profiles, interactions } = useData();
  const { user } = useAuth();
  ...
  const mutualMatches = useMemo(() => {
    ...
    const likedMe = interactions
      .filter(i => i.toUserId === user.id && i.type === 'like')
      .map(i => i.fromUserId);

    // Find users I liked
    const iLiked = interactions
      .filter(i => i.fromUserId === user.id && i.type === 'like')
      .map(i => i.toUserId);

    // Intersection
    const mutualUserIds = likedMe.filter(id => iLiked.includes(id));

    return profiles
      .filter(p => mutualUserIds.includes(p.userId))
      .map(profile => {
        let score = 0;
        const details = [];
  ...
  }, [profiles, interactions, myProfile, user]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {mutualMatches.map(({ profile, score, details }) => (
        ...
        {mutualMatches.length === 0 && (
          <div className="col-span-full py-12 text-center text-stone-500 bg-white rounded-2xl border border-stone-200 border-dashed">
            No mutual matches yet. Go to Discover to find potential roommates!
          </div>
        )}
      </div>
    </div>
